 ...
 * @downloadUrl https://raw.githubusercontent.com/paodequeijo616/GoLiveBypass_BetterDiscord/main/GoLiveDeQueijo.plugin.js a
 */
...
module.exports = class GoLiveBypassBD {
    ...
    getFullDiagnostic() {
        ...
        return [
            ...
            "A v1.9.9 usa DIRECT-FIRST: o Discord fica direto por padrão, sem proxy/ping extra. O catálogo SOCKS5/TUN vira fallback e só é acionado diante de falha real de vídeo ou bloqueio explícito do servidor."
        ].join("\n");
    }
    ...
    getSettingsPanel() {
                        ...
                        status?.state === "auto-proxy-search"
                            ? (
                                status?.autoProxyBestUdpMs
                                    ? `melhor fallback: ${status.autoProxyBestCountry || "?"} • ${status.autoProxyBestUdpMs}ms`
                                    : `fallback procurando • catálogo ${status.autoCatalogCount || 0}`
                            )
                            : "direta • fallback TUN automático"
                    );
        ...
        const directRemoteOk =
            status?.directRemoteFirstFrame === true;

        const directOutboundOk =
            status?.directOutboundActive === true;
        ...
        const stateText =
            directFirst && autoSearching
                ? `FALLBACK • BUSCANDO TUN ${status?.autoProxyTested || 0}/${status?.autoProxyCandidates || "?"}`
                : directFirst && directRemoteOk
                    ? "DIRETO • LIVE RECEBIDA"
                    : directFirst && directOutboundOk
                        ? "DIRETO • STREAM ATIVO"
                        : directFirst
                            ? "DIRETO • TUN EM ESPERA"
                            : singBoxReady && proxyHealth?.ok === false
                                ? proxyHealth.label
                                : singBoxReady
                                    ? "TUN ATIVO • TCP + UDP"
                                    : autoSearching
                                        ? `BUSCANDO ROTAS • ${status?.autoProxyTested || 0}/${status?.autoProxyCandidates || "?"} • fontes ${status?.autoSourceCount || 0}`
        ...
        return React.createElement(
            ...
            React.createElement(
                ...
                React.createElement("h2", {style: {margin: 0}}, "Go Live De Queijo v1.9.9"),
                React.createElement(
                    "div",
            ...
            React.createElement(
                ...
                React.createElement(
                    ...
                    React.createElement(
                        ...
                        React.createElement("div", {style: {fontSize: "12px", color: "var(--text-muted)", marginBottom: "4px"}}, "Rota"),
                        React.createElement(
                            "div",
                            {style: {fontSize: "15px", fontWeight: 700}},
                            directFirst && !singBoxTunUp
                                ? "DIRECT"
                                : singBoxTunUp
                                    ? (status?.singBoxTunName || singBoxStatus?.interfaceName || "GoLiveDeQueijo")
                                    : "-"
                        )
                    ),
                    React.createElement(
                    ...
                    React.createElement(
                        ...
                        React.createElement(
                            "div",
                            {style: {fontSize: "15px", fontWeight: 700}},
                            (runtimeLoaded || status?.hookLoaded === true)
                                ? "Carregado"
                                : "Pendente"
                        )
                    )
            ...
            React.createElement(
                ...
                field(
                    "Saída TCP/UDP",
                    "Catálogo global ampliado. Busca shards da América do Sul/Américas primeiro, usa várias fontes extras e continua de onde parou entre reinícios. ≤150 ms usa na hora.",
