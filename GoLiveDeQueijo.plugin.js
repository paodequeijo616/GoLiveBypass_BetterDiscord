...
module.exports = class GoLiveBypassBD {
    ...
    getFullDiagnostic() {
        ...
        return [
            ...
            "A v1.9.13 adiciona Media Route Race para Go Live e câmera: prepara rotas de baixa latência em background, mede RTT/jitter/perda e só troca o DIRECT se a rota for claramente melhor."
        ].join("\n");
    }
    ...
    getSettingsPanel() {
                  ...
                  status?.state !== "auto-proxy-search"
                    ? (
                        status?.routeBestAvgMs
                            ? `DIRECT • reserva ${status.routeBestCountry || "?"} ${status.routeBestAvgMs}ms`
                            : "DIRECT • procurando rota reserva"
                    )
                    : (
                        status?.state === "auto-proxy-search"
                            ? (
            ...
            directFirst && autoSearching
                ...
                : directFirst && directRemoteOk
                    ...
                    : directFirst && directOutboundOk
                        ? (
                            status?.directMediaType === "camera"
                                ? `DIRETO • CÂMERA ${status?.directMediaRttMs ? status.directMediaRttMs + "ms" : "ATIVA"}`
                                : `DIRETO • GOLIVE ${status?.directMediaRttMs ? status.directMediaRttMs + "ms" : "ATIVO"}`
                        )
                        : directFirst
                            ? "DIRETO • RESET LIMPO"
        ...
        return React.createElement(
            ...
            React.createElement(
                ...
                React.createElement("h2", {style: {margin: 0}}, "Go Live De Queijo v1.9.13"),
                React.createElement(
                    "div",
                ...
                React.createElement(
                    ...
                    React.createElement(
                        "button",
                        {
                            ...
                            onClick: () => plugin.confirmFullUninstall()
                        },
                        "Limpar integração"
                    )
                )
