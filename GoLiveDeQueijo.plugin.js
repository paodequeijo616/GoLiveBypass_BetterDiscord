...
module.exports = class GoLiveBypassBD {
    ...
    async verifyServerSessionAndRetry() {
        ...
        if (
            ...
            BdApi.UI.showToast(
                "Go Live De Queijo: servidor ainda bloqueou esta sessão e a proxy ainda não está pronta.",
        ...
        this.sessionVerdict += ` • recarregando (${next}/2)`;

        BdApi.UI.showToast(
            `Go Live De Queijo: servidor ainda bloqueou; recarregando atrás da proxy (${next}/2).`,
    ...
    githubRawUpdateUrl() {
        const repo = this.normalizeGithubRepo(this.settings.githubRepo);
    ...
    async checkForUpdates(manual = false) {
        ...
        if (!url) {
            if (manual) {
                BdApi.UI.alert(
                    "Go Live De Queijo",
                    "Configure primeiro o repositório em Atualizações.\n\nExemplo: usuario/repositorio"
                );
            }
    ...
    startAutoUpdater() {
        ...
        if (!this.normalizeGithubRepo(this.settings.githubRepo)) return;
    ...
    getSettingsPanel() {
        ...
        return React.createElement(
            ...
            React.createElement(
                ...
                React.createElement("h2", {style: {margin: 0}}, "Go Live De Queijo v1.6.0"),
                React.createElement(
                    "div",
                    {style: {fontSize: "12px", color: "var(--text-muted)"}},
                    "Configurações da proxy nativa do Gateway"
                )
            ),
            ...
            React.createElement(
                ...
                field(
                    "Proxy SOCKS5 manual",
                    "Opcional. Se vazio, a busca continua automática.",
                    React.createElement("input", {
                        style: inputStyle,
                ...
                field(
                    "Repositório GitHub",
                    "Usado para buscar novas versões. Ex.: usuario/repositorio",
                    React.createElement("input", {
                        style: inputStyle,
                        value: this.settings.githubRepo,
                        placeholder: "usuario/repositorio",
                        onChange: e => save("githubRepo", e.target.value.trim()),
                        onBlur: () => plugin.startAutoUpdater()
                    })
                ),

                field(
                    "Atualização automática",
                    "Verifica uma vez ao abrir e depois a cada 6 horas.",
                    React.createElement(
                        "div",
                        {style: toggleWrapStyle},
                        React.createElement(
