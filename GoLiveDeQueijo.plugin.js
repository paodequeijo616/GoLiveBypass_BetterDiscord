...
module.exports = class GoLiveBypassBD {
    ...
    getFullDiagnostic() {
        ...
        return [
            ...
            "A v1.9.8 amplia as fontes SOCKS5, consulta shards por país perto do Brasil e corrige a retomada: proxies recém-falhadas vão para o fim e as ainda não testadas entram primeiro."
        ].join("\n");
    }
    ...
    getSettingsPanel() {
                    ...
                    status?.state === "auto-proxy-search"
                        ? (
                            status?.autoProxyBestUdpMs
                                ? `melhor: ${status.autoProxyBestCountry || "?"} • ${status.autoProxyBestUdpMs}ms • catálogo ${status.autoCatalogCount || 0}`
                                : `catálogo ${status.autoCatalogCount || 0} • testando países próximos...`
                        )
        ...
        return React.createElement(
            ...
            React.createElement(
                ...
                React.createElement("h2", {style: {margin: 0}}, "Go Live De Queijo v1.9.8"),
                React.createElement(
                    "div",
