...
let _childProcessMethod =
    null;

function getChildProcess() {
    if (
        _childProcess &&
        typeof _childProcess.spawn ===
            "function"
    ) {
    ...
    const attempts =
        [
            ...
            [
                "Module._load(node:child_process)",
                () =>
...
function getElectronShell() {
    ...
    for (
        ...
        try {
            ...
            if (
                electron?.shell &&
                typeof electron.shell.openPath ===
                    "function"
            ) {
...
module.exports = class GoLiveBypassBD {
    ...
    log(message, data = null) {
        ...
        try {
            if (
                typeof this.appendRendererLog ===
                "function"
            ) {
                this.appendRendererLog(
                    "plugin.log",
                    {
                        message:
                            text,
    ...
    start() {
        ...
        // O registerPreloadScript do Electron não estava entrando no
        // renderer de voz desta build do Discord. Tentamos também pelo
        // renderer atual do BetterDiscord, onde DiscordNative existe.
        for (
            const delay of [
                150,
                1200,
        ...
        this.schedule(
            () => {
                return this.verifyServerSessionAndRetry();
            },
            1800
        );
    ...
    get routeBenchmarksPath() {
        return path.join(this.dataDir, "route-benchmarks.json");
    }

    get routeCommandPath() {
        return path.join(this.dataDir, "route-command.json");
    }

    get routeCommandResultPath() {
        return path.join(this.dataDir, "route-command-result.json");
    }
        ...
        try {
            ...
            fs.writeFileSync(
                ...
                JSON.stringify({
                    ...
                    manualProxy: "",
                    networkMode: DIRECT_ONLY_MODE ? "direct-first" : "singbox-auto",
                    singBoxProxy: "",
                    autoRecoverVideo: this.settings.autoRecoverVideo !== false,
                    autoRecoverViewer: this.settings.autoRecoverViewer !== false,
                    forceViewerVideoSupported: true,
                    foreignRouteOnBlockedSession: FOREIGN_ROUTE_ON_BLOCKED_SESSION,
                    routeMode: String(this.settings.routeMode || "auto"),
                    preferredRoute: String(this.settings.preferredRoute || ""),
    ...
    writeRouteCommand(type, extra = {}) {
        ...
        fs.writeFileSync(
            this.routeCommandPath,
            JSON.stringify(command, null, 2),
            "utf8"
        );

        this.log(`ROUTE UI command: ${type}`);
        return command;
    }

    requestRouteBenchmark() {
        this.writeRouteCommand(
            "benchmark-routes"
        );

        BdApi.UI.showToast(
            "Testando rotas: ping + download + upload. Isso pode levar alguns segundos.",
            {
                type: "info",
                timeout: 7000
            }
    ...
    applyPreferredRoute(raw) {
        ...
        if (
            ...
            BdApi.UI.alert(
                PLUGIN_NAME,
                "Rota SOCKS5 inválida."
            );
            return;
        }

        // Não salva como preferida antes da validação.
        // Se a proxy morrer, a configuração anterior continua intacta.
        this.writeRouteCommand(
            "apply-route",
            {
                proxy:
        ...
        BdApi.UI.showToast(
            "Validando e aplicando a rota escolhida. Se ela falhar, a rota atual será mantida.",
            {
                type:
                    "info",
                timeout:
    ...
    useAutomaticRoute() {
        ...
        BdApi.UI.showToast(
            "Modo automático ativado. A melhor rota será escolhida pela nota combinada.",
            {
                type: "success",
                timeout: 5500
            }
    ...
    requestNativeFallback(
        ...
        const foreignRouteReason =
            /servidor bloqueou Go Live|remote-stream|viewer/i.test(
                text
            );
        ...
        if (
            ...
            this.log(
                `DIRECT-FIRST: fallback de qualidade ignorado (${text})`
            );
        ...
        try {
            ...
            if (native?.singBoxReady === true) return true;
            try {
                const pending = JSON.parse(fs.readFileSync(this.singBoxCommandPath, "utf8"));
                if (["fallback-now", "auto-bootstrap", "bootstrap"].includes(pending?.type) &&
                    Date.now() - Date.parse(pending.at || "") < 180000) return true;
            } catch {}
            this.writeSingBoxSupportFiles();
            ...
            this.log(
                `rota estrangeira TUN solicitada: ${command.reason}`
            );

            return true;
        } catch (e) {
            this.log(
                `rota estrangeira TUN não pôde ser solicitada: ${e.message}`
            );
        ...
        for (const target of definitions) {
            try {
                ...
                this.videoGuardOriginals.push({
                    target,
                    variations: original,
                    descriptors: Object.getOwnPropertyDescriptors(original)
                });
    ...
    installNarrowMediaGateFallback() {
        ...
        try {
            ...
            // Go Live / vídeo dentro do app.
            if (
                typeof media.supportsInApp ===
                    "function"
            ) {
            ...
            // Câmera e a capacidade de vídeo que alimenta
            // videoSupported na camada discord_voice.
            //
            // O log real mostrou supports("VIDEO") = false mesmo
            // nesta máquina já tendo exibido primeiro frame de vídeo
            // em versões anteriores. Portanto isso é um gate do cliente,
            // não falta de decoder/hardware.
            if (
                typeof media.supports ===
                    "function"
            ) {
                this.api.Patcher.after(
                    media,
            ...
            // DESKTOP_CAPTURE continua ORIGINAL.
            // Não forçamos outros tipos/capacidades.
            this.mediaGateFallbackInstalled =
                patched > 0;
            ...
            if (
                ...
                this.log(
                    "VIDEO CAPABILITY: supports(VIDEO) + supportsInApp(VIDEO) liberados; DESKTOP_CAPTURE original."
                );
            }
            ...
            this.api.Logger.warn(
                "Fallback VIDEO capability falhou",
                e
            );
    ...
    installVideoGuardPatch() {
        ...
        const count =
            this.patchVideoGuardDefinitions();
        ...
        try {
            ...
            if (
                ...
                this.originalApexGetServerAssignment =
                    apex.getServerAssignment.bind(
                        apex
                    );
            }

            // IMPORTANTE:
            // a 2.1.0 só aplicava este override quando não achava a
            // declaração variations:{}.
            //
            // No Discord atual nós encontramos a declaração, mas o store
            // continua devolvendo o variantId real e consumidores já
            // cacheados continuam fechando o Go Live.
            //
            // No modo DIRECT aplicamos os patches locais em conjunto:
            // 1) variations:{} na declaração
            // 2) getServerAssignment(VIDEO_GUARD) -> null
            //
            // O override é estreito e só afeta 2026-08-video-guard.
            if (
                apex?.getServerAssignment
            ) {
            ...
            this.api.Logger.warn(
                "Override estreito do ApexExperimentStore falhou",
                e
            );
        }
        ...
        this.videoGuardPatchMode =
            modes.length
                ...
                : "patch local não aplicado";

        // O MediaEngineStore é a última trava LOCAL.
        // Mantemos o patch estreito: somente supportsInApp("VIDEO").
        this.schedule(
            () => {
                if (
                    !this.settings.enabled
                ...
                try {
                    ...
                    if (
                        ...
                        const ok =
                            this.installNarrowMediaGateFallback();

                        if (
                            ok
                        ) {
                            this.videoGuardPatchMode +=
                                " + supports(VIDEO) + supportsInApp(VIDEO)";

                            BdApi.UI.showToast(
                                "Go Live De Queijo: câmera e Go Live liberados no modo DIRECT.",
                                {
                                    type:
                                        "success",
                                    timeout:
    ...
    restoreVideoGuardPatch() {
        for (const item of this.videoGuardOriginals || []) {
            try {
                ...
                if (item.target.variations === item.variations) {
                    Object.defineProperties(item.variations, item.descriptors);
                }
            } catch {}
        }
    ...
    scheduleServerProxyRecheck() {
        ...
        if (this._serverProxyWaitChecks >= 40) {
            ...
            this.sessionVerdict += this.lastServerAssignment === undefined
                ? " • atribuição indisponível após 80s"
                : singBoxMode
    ...
    async verifyServerSessionAndRetry() {
        if (this._stopped || !this.settings.enabled) return;
        ...
        if (
            ...
            this.sessionVerdict =
                `atribuição REAL variantId ${assignment.variantId} • override local DIRECT ativo`;

            this._serverProxyWaitChecks =
                0;

            this._serverProxyWaitScheduled =
                false;

            this.appendRendererLog(
                "server.assignment.direct-override",
                {
                    variantId:
        ...
        if (
            ...
            this.sessionVerdict =
                `atribuição REAL variantId ${assignment.variantId} • flags locais liberadas • rota estrangeira solicitada`;

            this.appendRendererLog(
                "server.assignment.foreign-route",
                {
                    variantId:
                        assignment.variantId,
                    realAssignment:
                    ...
                    excludedCountries:
                        String(
                            this.settings.excludedCountries ||
                            "BR"
                        )
                }
            );
        ...
        if (directFirstMode) {
            if (native?.singBoxReady === true) {
                this.sessionVerdict = `rota estrangeira ativa (${native.autoProxyCountry || "?"}); TCP DIRECT`;
                return;
            }
            ...
            this.sessionVerdict =
                `BLOQUEADA pelo servidor (variantId ${assignment.variantId}) • aguardando saída estrangeira`;
        ...
        if (singBoxMode) {
            ...
            this.schedule(() => {
                if (!this.settings.enabled || this.hasActiveStreamMedia()) {
                    this.api.Data.save("sessionRetryState", retry);
                    return;
                }
    ...
    async quitDiscordWithoutRelaunch() {
        // Compatibilidade para chamadas antigas.
        // Desde a v2.1.8 o helper elevado é responsável por fechar o Discord.
        this.log(
            "quitDiscordWithoutRelaunch ignorado: fechamento é responsabilidade do restart helper."
        );

        return true;
    }

    async cleanupLegacySingBoxBeforeRestart() {
        if (
        ...
        const helperPath =
            path.join(
                base,
                "direct-restart-helper.ps1"
            );

        const resultPath =
            path.join(
                base,
                "direct-restart-helper-result.json"
            );

        const helperLogPath =
            path.join(
                base,
                "direct-restart-helper.log"
            );
...
$Base = Join-Path $env:LOCALAPPDATA "GoLiveBypassBD"
$TaskName = "GoLiveDeQueijo-SingBox"
$TargetSingBox = [IO.Path]::GetFullPath((Join-Path $Base "sing-box\sing-box.exe"))
...
function Append-Log([string]$Message) {
    try {
        Add-Content -LiteralPath $Log -Value ("{0:o} [restart-helper] {1}" -f (Get-Date), $Message) -Encoding UTF8
    } catch {}
}

function Write-Result([string]$Stage, [bool]$Ok, [hashtable]$Extra) {
    $obj = [ordered]@{
        version = "2.1.6"
        stage = $Stage
        ok = [bool]$Ok
    ...
    Write-Text $Result ($obj | ConvertTo-Json -Depth 8 -Compress)
}
...
if (-not (Is-Administrator)) {
    ...
    try {
        $args = '-NoProfile -ExecutionPolicy Bypass -File "' + $PSCommandPath + '" -DiscordExe "' + $DiscordExe + '" -UpdateExe "' + $UpdateExe + '" -DiscordRoot "' + $DiscordRoot + '" -DiscordPid ' + [string]$DiscordPid + ' -Result "' + $Result + '" -Log "' + $Log + '"'
        Start-Process -FilePath $PowerShellExe -Verb RunAs -ArgumentList $args -WindowStyle Hidden | Out-Null
        exit 0
    } catch {
        Append-Log ("UAC falhou: " + $_.Exception.Message)
        Write-Result "uac-failed" $false @{ error = $_.Exception.Message }
        exit 10
    }
}

Append-Log "helper elevado armado"

Write-Result "armed" $true @{
    discordPid = [int]$DiscordPid
    discordExe = $DiscordExe
    updateExe = $UpdateExe
}

# Dá tempo para o plugin enxergar stage=armed e mostrar o toast.
Start-Sleep -Milliseconds 1800

Append-Log ("encerrando árvore do Discord PID " + [string]$DiscordPid)

# O renderer desta build não expõe DiscordNative.app.quit/exit.
# Portanto o próprio helper elevado encerra a árvore do processo.
try {
    & taskkill.exe /PID $DiscordPid /T /F | Out-Null
} catch {}

Start-Sleep -Milliseconds 500

if (Get-Process -Id $DiscordPid -ErrorAction SilentlyContinue) {
    try {
        Stop-Process -Id $DiscordPid -Force -ErrorAction SilentlyContinue
    } catch {}
}

# Também elimina qualquer Discord.exe restante pertencente a esta instalação.
# Isso acontece ANTES de Update.exe ser iniciado, então não atingimos uma
# instância nova criada pelo próprio helper.
for ($attempt = 0; $attempt -lt 8; $attempt++) {
    $discordLeft = @(
        Get-CimInstance Win32_Process -Filter "Name='Discord.exe'" -ErrorAction SilentlyContinue |
        Where-Object {
            $exe = $null
            ...
            $exe -and
            $exe.StartsWith(
                [IO.Path]::GetFullPath($DiscordRoot),
                [System.StringComparison]::OrdinalIgnoreCase
            )
        }
    )

    if ($discordLeft.Count -eq 0) {
        break
    }

    foreach ($proc in $discordLeft) {
        try {
...
$originalStillAlive =
    [bool](
        Get-Process -Id $DiscordPid -ErrorAction SilentlyContinue
    )

if ($originalStillAlive) {
    Append-Log "não consegui encerrar o PID original do Discord"

    Write-Result "close-failed" $false @{
        discordPid = [int]$DiscordPid
    }
...
Append-Log "Discord fechado pelo helper; iniciando limpeza"

try {
    Stop-ScheduledTask -TaskName $TaskName -ErrorAction SilentlyContinue
} catch {}

# A task atual não possui trigger. Mantemos registrada e PARADA
# para poder reutilizar a rota no próximo boot sem novo UAC.
Start-Sleep -Milliseconds 300
...
for ($attempt = 0; $attempt -lt 8; $attempt++) {
    $procs = @(
        ...
        Where-Object {
            ...
            ($exe -and $exe -ieq $TargetSingBox) -or
            ($cmd -and $cmd -like "*GoLiveBypassBD*sing-box.exe*")
        }
    )
...
foreach ($name in @("singbox-command.json", "singbox-command-result.json")) {
    try {
        Remove-Item -LiteralPath (Join-Path $Base $name) -Force -ErrorAction SilentlyContinue
    } catch {}
}
...
if ($adapter) {
    try {
        $routes = @(Get-NetRoute -InterfaceIndex $adapter.ifIndex -PolicyStore ActiveStore -ErrorAction SilentlyContinue)
...
$adapterAfter = Get-NetAdapter -ErrorAction SilentlyContinue |
    Where-Object {
        $_.Name -like "GoLiveDeQueijo*" -or
        $_.InterfaceDescription -like "*sing-box*"
    } |
...
$tunUpAfter = [bool]($adapterAfter -and $adapterAfter.Status -eq "Up")

$blocking = @()

$taskRunningAfter = [bool]($taskAfter -and $taskAfter.State -eq "Running")
if ($taskRunningAfter) { $blocking += "task-running" }
if ($runnerAfter.Count -gt 0) { $blocking += "runner" }
if ($singAfter.Count -gt 0) { $blocking += "sing-box" }
if ($tunUpAfter) { $blocking += "tun-up" }

$clean = [bool]($blocking.Count -eq 0)

Append-Log ("limpeza: clean=" + $clean + " task=" + [bool]$taskAfter + " runner=" + $runnerAfter.Count + " singBox=" + $singAfter.Count + " tunUp=" + $tunUpAfter)

Write-Result "cleanup-complete" $clean @{
    taskExistsAfter = [bool]$taskAfter
    taskRunningAfter = [bool]$taskRunningAfter
    runnerCountAfter = [int]$runnerAfter.Count
    singBoxCountAfter = [int]$singAfter.Count
    tunUpAfter = [bool]$tunUpAfter
    blockingReasons = @($blocking)
    runnerKilled = [int]$runnerKilled
    singBoxKilled = [int]$singKilled
    routeRemoved = [int]$routeRemoved
    adapterDisabled = [bool]$adapterDisabled
}

try {
    if ($clean -and (Test-Path -LiteralPath $UpdateExe)) {
        Append-Log "iniciando Discord via Update.exe"
        Start-Process -FilePath $UpdateExe -ArgumentList @("--processStart", [IO.Path]::GetFileName($DiscordExe)) | Out-Null
    } elseif (Test-Path -LiteralPath $DiscordExe) {
        Append-Log "fallback: iniciando Discord.exe diretamente"
        Start-Process -FilePath $DiscordExe | Out-Null
    } else {
        Append-Log "não encontrei executável para reiniciar Discord"
    }
} catch {
    Append-Log ("falha reiniciando Discord: " + $_.Exception.Message)
}
...
`;
        ...
        const loaderStatusPath =
            path.join(
                base,
                "direct-restart-loader.json"
            );
        ...
        if (
            ...
            // Fallback sem child_process:
            // ShellExecute do Electron abre um .cmd que inicia o PowerShell.
            const shell =
                getElectronShell();
            ...
            if (
                ...
                throw new Error(
                    "Não consegui acessar child_process.spawn nem electron.shell.openPath no BetterDiscord."
                );
            }
            ...
            if (
                ...
                throw new Error(
                    "electron.shell.openPath falhou: " +
                    openResult
                );
            }

            launchMethod =
                "electron.shell.openPath(.cmd)";
        }
        ...
        this.log(
            `DIRECT restart helper lançado via ${launchMethod}`
        );
        ...
        while (
            ...
            if (
                ...
                this.log(
                    "DIRECT restart helper armado com elevação; Discord pode fechar com segurança."
                );
            ...
            if (
                last?.stage === "uac-failed"
            ) {
                throw new Error(
                    "A elevação do helper foi cancelada/falhou: " +
                    (last?.error || "sem detalhes")
                );
            }
        ...
        throw new Error(
            "O helper de restart não confirmou stage=armed em 20s. " +
            `Método=${loaderInfo?.launchMethod || launchMethod || "?"}. ` +
            "O Discord continuará aberto; nenhuma reinicialização foi iniciada. " +
            "Consulte direct-restart-helper.log e direct-restart-loader.json."
        );
    }
    ...
    async installAndRestart() {
        ...
        try {
            ...
            BdApi.UI.showToast(
                "Preparando atualização DIRECT segura...",
                {
                    type: "info",
                    timeout: 7000
                }
            ...
            const helper =
                await this.cleanupLegacySingBoxBeforeRestart();
            ...
            if (
                ...
                throw new Error(
                    "O helper de restart não ficou armado."
                );
            }

            BdApi.UI.showToast(
                `Hook ${PLUGIN_VERSION} instalado em ${target.appName}. ` +
                "O helper elevado vai fechar o Discord, limpar o sing-box/TUN e abrir novamente.",
                {
                    type: "success",
                    timeout: 9000
                }
            ...
            this.log(
                "DIRECT restart: helper armado; fechamento do Discord entregue ao helper elevado."
            );

            // NÃO chamar DiscordNative.app.quit/exit:
            // nesta build esses métodos não existem.
            // O helper já está independente/elevado e fechará este PID.
            return true;
        } catch (e) {
            this.api.Logger.error(
                "Falha ao preparar modo DIRECT/reiniciar",
                e
            );

            BdApi.UI.alert(
                PLUGIN_NAME,
                "Não consegui preparar o modo DIRECT:\n\n" +
                (e?.message || String(e))
            );
        ...
        try {
            if (
                ...
                if (
                    ...
                    BdApi.UI.showToast(
                        "Hook removido. O helper vai fechar o Discord, limpar resíduos e abrir novamente...",
                        {
                            type: "info",
                            timeout: 8000
                        }
                    ...
                    this.log(
                        "removeAndRestart: fechamento entregue ao helper elevado."
                    );
            ...
            BdApi.UI.alert(
                "Go Live De Queijo",
                "Hook foi removido, mas não consegui preparar o restart seguro. " +
    ...
    openActivationTutorial(options = {}) {
        ...
        const steps = [
            ...
            },
            {
                ...
                title: "Riscos e confirmação",
                text: "Risco de punição: baixo, mas não zero. O BetterDiscord e este plugin modificam o cliente do Discord, então existe possibilidade de suspensão ou ban. Essa classificação é apenas uma avaliação prática do projeto e não é uma garantia do Discord.",
                points: [
                    "O plugin não usa self-bot, spam ou automação de mensagens, mas isso não elimina o risco de modificar o cliente.",
                    "O Discord pode alterar regras, funcionamento ou formas de detecção a qualquer momento.",
                    "Proxies públicas são serviços de terceiros e podem ficar lentas, instáveis ou indisponíveis.",
                    "Ao continuar, você confirma que entendeu os riscos. A integração pode ser pausada ou limpa pelo próprio painel."
                ]
            }
        ];
    ...
    installRendererVoiceHook() {
        ...
        try {
            ...
            if (
                ...
                // Um preload que lançou exceção podia deixar a flag
                // eternamente em "loading" e impedir todos os retries.
                this.log(
                    "voice renderer hook ficou preso em loading; resetando sentinel e tentando novamente"
                );

                try {
                    delete globalThis.__GLQ_VOICE_2210__;
                } catch {
                    globalThis.__GLQ_VOICE_2210__ =
                        null;
                }
            ...
            if (
                !fs.existsSync(
                    this.voicePreloadPath
                )
            ) {
                this.log(
                    "voice renderer hook: voice-preload.js ainda não existe"
                );

                return false;
            }

            // Executa exatamente o mesmo arquivo que o main-hook registra
            // como preload, mas agora diretamente no renderer onde o
            // BetterDiscord já está rodando e DiscordNative está disponível.
            try {
                const resolved =
                    require.resolve(
                        this.voicePreloadPath
            ...
            if (
                ...
                this.log(
                    "voice renderer hook carregado diretamente pelo BetterDiscord"
                );
            }
            ...
            this.appendRendererLog(
                "voice.direct-renderer-hook.error",
                {
                    message:
                        e?.message ||
                        String(e),
            ...
            this.log(
                `voice renderer hook falhou: ${e?.message || e}`
            );
    ...
    getFullDiagnostic() {
        ...
        return [
            ...
            "A v2.1.0 adiciona um aviso curto de risco na etapa final do tutorial de ativação."
        ].join("\n");
    }
    ...
    getDiagnostic() {
        ...
        const lines = [
            `Go Live De Queijo v${PLUGIN_VERSION}`,
            "",
            "== NATIVE HOOK ==",
        ...
        if (!status) {
            ...
            lines.push(
                `UDP RTT (sonda): ${status.autoProxyUdpMs ?? "-"} ms`
            );
            lines.push(
                `Gateway TLS: ${status.autoProxyGatewayMs ?? "-"} ms`
            );
            lines.push(`RTT real Discord: ${status.voiceRttObservedMs ?? "-"} ms (meta 250 ms)`);
            lines.push(`Otimizador de ping: ${status.voiceRttOptimizerState || "aguardando mídia"}`);
            lines.push(`RTT validado após troca: ${status.voiceRttValidatedMs ?? "-"} ms`);
            lines.push(`Erro otimizador: ${status.voiceRttOptimizerError || "-"}`);
            lines.push(`Erro rollback: ${status.voiceRttRollbackError || "-"}`);
            lines.push(
                `Download rota: ${status.autoProxyDownloadMbps ?? "-"} Mbps`
            );
            lines.push(
                `Upload rota: ${status.autoProxyUploadMbps ?? "-"} Mbps`
            );
            lines.push(
                `Nota ponderada: ${status.autoProxyWeightedScore ?? "-"}`
            );
            lines.push(
        ...
        lines.push(
            ...
            "MediaEngineStore: supports(VIDEO) + supportsInApp(VIDEO) liberados; DESKTOP_CAPTURE original"
        );
    ...
    getSettingsPanel() {
        ...
        function Panel() {
            ...
            const routeBench =
                plugin.readRouteBenchmarks();

            const routeEntries =
                Array.isArray(routeBench.entries)
                    ? routeBench.entries
                    : [];
                        ...
                        React.createElement(
                            ...
                            React.createElement(
                                ...
                                React.createElement(
                                    ...
                                    React.createElement(
                                        ...
                                        "Nota: 50% ping + 25% download + 25% upload. Os testes usam amostras pequenas para não gastar tráfego demais."
                                    )
                                ),
                            ...
                            routeEntries.length
                                        ...
                                        .map(
                                            (item, index) => {
                                                ...
                                                const selected =
                                                    plugin.settings.routeMode ===
                                                        "manual" &&
                                                    plugin.settings.preferredRoute ===
                                                        raw;
                                ...
                                : React.createElement(
                                    ...
                                    routeBench.state ===
                                        "running"
                                        ? "Testando rotas. A lista vai aparecer automaticamente."
                                        : "Ainda não há benchmark salvo. Clique em “Reavaliar rotas”."
                                )
                        ),
