...
module.exports = class GoLiveBypassBD {
    ...
    async selectWireGuardConfig() {
        if (process.platform !== "win32") {
            BdApi.UI.alert(
                PLUGIN_NAME,
                "A integração WireGuard automática da v1.7.1 é para Windows."
            );
            return null;
        }
        ...
        const script = [
            ...
            "$d.Filter = 'WireGuard config (*.conf)|*.conf|Todos os arquivos (*.*)|*.*';",
            "$d.Title = 'Selecione a configuração WireGuard/Proton VPN';",
            "$d.Multiselect = $false;",
        ...
        try {
            ...
            this.validateWireGuardConfig(selected);

            this.settings.wireGuardConfigPath = selected;
            this.settings.wireGuardConfigured = false;
            this.saveSettingsAndNativeConfig();

            BdApi.UI.showToast(
                "Config WireGuard selecionada. Clique em Instalar/atualizar e reiniciar.",
            ...
            BdApi.UI.alert(
                PLUGIN_NAME,
                `Não consegui selecionar/validar a config:\n\n${e.message}`
            );
    ...
    validateWireGuardConfig(file) {
        ...
        if (peers !== 1) {
            throw new Error(
                "v1.7.1 aceita exatamente 1 [Peer] por configuração"
            );
        }
    ...
    writeWireGuardSupportFiles() {
        ...
        const sourcePath = String(
            this.settings.wireGuardConfigPath || ""
        ).trim();

        if (!sourcePath) {
            throw new Error(
                "selecione primeiro um arquivo WireGuard/Proton .conf"
            );
        }
    ...
    async ensureWireGuardReady() {
        if (process.platform !== "win32") {
            throw new Error(
                "WireGuard automático da v1.7.1 suporta Windows"
            );
        }
        ...
        if (!selected) {
            throw new Error(
                "Selecione a config Proton/WireGuard nas configurações do plugin."
            );
        }

        // Always rebuild/reapply on install/update. This keeps the config and
        // helper current after plugin updates while still asking for UAC only
        // for the explicit install/update action.
    ...
    async installAndRestart() {
        try {
            ...
            BdApi.UI.showToast(
                "Preparando túnel WireGuard do Discord...",
                {type: "info", timeout: 7000}
            );
    ...
    getFullDiagnostic() {
        ...
        return [
            ...
            "",
            "NOTA v1.7.1:",
            "Os dois logs da v1.7.1 confirmaram remote-stream com videoSupported=true e callback de primeiro frame instalado, mas nenhum frame chegou.",
            "Fechar/reabrir *.discord.media também não resolveu. Por isso a v1.7.1 testa outra hipótese: a sinalização *.discord.media passa pela mesma saída não-BR do Gateway.",
    ...
    getSettingsPanel() {
        ...
        return React.createElement(
            ...
            React.createElement(
                ...
                React.createElement("h2", {style: {margin: 0}}, "Go Live De Queijo v1.7.1"),
                React.createElement(
                    "div",
            ...
            React.createElement(
                ...
                field(
                    ...
                    "Selecione um .conf WireGuard. O plugin instala o cliente oficial automaticamente e cria rotas somente para o Discord.",
                    React.createElement(
                        "div",
                        {
                            style: {
                        ...
                        React.createElement("input", {
                            ...
                            value: this.settings.wireGuardConfigPath || "",
                            placeholder: "Nenhum .conf selecionado",
                            readOnly: true
                        }),
                        React.createElement(
                            "button",
                            {
                                ...
                                onClick: () => plugin.selectWireGuardConfig()
                            },
                            "Selecionar .conf"
                        )
                    )
