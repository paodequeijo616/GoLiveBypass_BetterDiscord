...
module.exports = class GoLiveBypassBD {
    ...
    get wireGuardPendingConfigPath() {
        return path.join(this.dataDir, "GLQVPN.pending.conf");
    }
    ...
    validateWireGuardConfig(file) {
        ...
        if (peers !== 1) {
            throw new Error(
                "v1.7.7 aceita exatamente 1 [Peer] por configuração"
            );
        }
    ...
    writeWireGuardSupportFiles() {
        ...
        try {
            fs.writeFileSync(
                this.wireGuardPendingConfigPath,
                managed,
                "utf8"
            );
        } catch (e) {
            throw new Error(
                "Não consegui criar a configuração temporária do WireGuard em:\n" +
                this.wireGuardPendingConfigPath +
                "\n\n" +
                e.message
            );
    ...
    runWireGuardBootstrap() {
        ...
        this.settings.wireGuardConfigured = false;
        this.saveSettingsAndNativeConfig();

        return command;
    }

    ensureWireGuardReady() {
        if (process.platform !== "win32") {
            throw new Error(
                "WireGuard automático da v1.7.7 suporta Windows"
            );
        }
    ...
    async installAndRestart() {
        try {
            ...
            BdApi.UI.showToast(
                "Preparando WireGuard e hook nativo...",
                {type: "info", timeout: 7000}
            );

            const command = this.ensureWireGuardReady();
            const target = this.ensureNativeHookFiles();

            BdApi.UI.showToast(
                `Config pronta • comando ${command.nonce.slice(-6)} • ` +
            ...
            BdApi.UI.alert(
                ...
                e.message +
                "\n\nA v1.7.7 executa a etapa elevada no processo nativo após o reinício do Discord."
            );
        }
    }
    ...
    getFullDiagnostic() {
        ...
        return [
            ...
            (() => {
                try {
                    return fs.readFileSync(
                        path.join(this.dataDir, "wg-command-result.json"),
                        "utf8"
                    ).replace(/^\uFEFF/, "");
                } catch {
            ...
            "",
            "NOTA v1.7.7:",
            "Os dois logs da v1.7.7 confirmaram remote-stream com videoSupported=true e callback de primeiro frame instalado, mas nenhum frame chegou.",
            "Fechar/reabrir *.discord.media também não resolveu. Por isso a v1.7.7 testa outra hipótese: a sinalização *.discord.media passa pela mesma saída não-BR do Gateway.",
    ...
    getSettingsPanel() {
        ...
        const wgInstalled = wgStatus?.wireGuardInstalled === true;
        const wgService = String(wgStatus?.tunnelService || "").toLowerCase();
        const wgStopped = wgInstalled && (
            wgService === "stopped" ||
            wgStatus?.tunnelUp === false
        );

        const stateText = wgReady
            ? `ATIVO • ${wgRoutes} rota${wgRoutes === 1 ? "" : "s"}`
            : wgStopped
                ? "INSTALADO • TÚNEL PARADO"
                : detectedWireGuardConfig
                    ? "AGUARDANDO ATIVAÇÃO"
        ...
        const stateColor = wgReady
            ? "var(--status-positive)"
            : detectedWireGuardConfig || wgInstalled
                ? "var(--status-warning)"
                : "var(--status-danger)";
        ...
        return React.createElement(
            ...
            React.createElement(
                ...
                React.createElement("h2", {style: {margin: 0}}, "Go Live De Queijo v1.7.7"),
                React.createElement(
                    "div",
            ...
            React.createElement(
                ...
                React.createElement(
                    ...
                    React.createElement(
                        ...
                        React.createElement("div", {style: {fontSize: "15px", fontWeight: 700}}, wgReady ? "GLQVPN ativo" : (wgStatus?.tunnelService || "-") )
                    ),
                    React.createElement(
                        "div",
                        {
